  ...
  public class PassthroughStylist : MonoBehaviour
  {
    ...
    IEnumerator SetTargetPassthroughStyle(PassthroughStyle newStyle, float lerpTime)
    {
      ...
      if (newStyle.PassthroughEdgeEnabled && !currentStyle.PassthroughEdgeEnabled)
      {
        ...
        currentStyle.PassthroughEdge = new Color(newStyle.PassthroughEdge.r, newStyle.PassthroughEdge.g,
          newStyle.PassthroughEdge.b, 0);
      }

      if (!newStyle.PassthroughEdgeEnabled && currentStyle.PassthroughEdgeEnabled)
      {
        newStyle.PassthroughEdge = new Color(currentStyle.PassthroughEdge.r, currentStyle.PassthroughEdge.g,
          currentStyle.PassthroughEdge.b, 0);
      }
      ...
      while (timer < lerpTime)
      {
        ...
        sceneOverlay.textureOpacity =
          Mathf.Lerp(currentStyle.PassthroughOpacity, newStyle.PassthroughOpacity, lerpValue);
        sceneOverlay.colorMapEditorContrast =
          Mathf.Lerp(currentStyle.PassthroughContrast, newStyle.PassthroughContrast, lerpValue);
        sceneOverlay.colorMapEditorBrightness = Mathf.Lerp(currentStyle.PassthroughBrightness,
          newStyle.PassthroughBrightness, lerpValue);
        sceneOverlay.colorMapEditorPosterize =
          Mathf.Lerp(currentStyle.PassthroughPosterize, newStyle.PassthroughPosterize, lerpValue);
    ...
    {
      sceneOverlay.colorMapEditorType = OVRPassthroughLayer.ColorMapEditorType.None;
